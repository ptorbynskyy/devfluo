...
export function setupEchoTool(server: McpServer): void {
	server.registerTool(
		"echo",
		{
			title: "Echo Tool",
			description: "Echo back the provided text",
			inputSchema: EchoToolZodSchema.shape,
		},
		async (args: EchoToolInput) => {
			return handleEchoTool(args);
		},
	);
}

export function setupCurrentTimeTool(server: McpServer): void {
	server.registerTool(
		"current_time",
		{
			title: "Current Time Tool",
			description: "Get the current date and time",
			inputSchema: CurrentTimeToolZodSchema.shape,
		},
		async (args: CurrentTimeToolInput) => {
			return handleCurrentTimeTool(args);
		},
	);
}
